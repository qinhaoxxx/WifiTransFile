$(function() {
    ...
    function fillFilesContainer() {
        // var height = $(window).height() - $("#left").height() - 200 - $("#right .content_title").height() - $("#right .table_header").height();
        var height = $("#left").height() * 2;

        var filesContainer = $("#right .files");
        filesContainer.empty();
        filesContainer.height(height);
        $("#file_wrapper").height = height + 1;
        var rowsCount = Math.floor(height / 20);
    ...
    function checkFileName(fileName) {
        ...
        if (!fileName || !fileName.toLowerCase().match('(tfbd)$')) {
    ...
    function bindAjaxUpload(fileSelector) {
        ...
        $(fileSelector).change(function() {
            ...
            $('.file_upload_warper').append('<input type="file" name="newfile" value="" id="newfile_' + uploadQueue.length + '" class="file_upload" webkitdirectory="" directory=""/>');
